    ...
    signInWithEmailAndPassword(auth, email, password)
      .then((userCredential) => {
        ...
        getDoc(doc(fireDb, 'users', user.uid)).then((user) => {
          localStorage.setItem('insta-lite', JSON.stringify({ ...user.data(), id: user.id }))
          navigate('/home')
    ...
    <div className='h-screen flex justify-between flex-col overflow-x-hidden'>
      ...
      <div className=" flex justify-start">
        <div className="h-40 bg-primary w-96 transform -skew-x-[25deg] -ml-10 flex items-center justify-center">
          <h1 className="text-center text-6xl font-semibold skew-x-[25deg] text-white "> Insta</h1>
        </div>
      </div>
      {/*form*/}
      <div className="flex justify-center">
        <div className='w-[420px] flex flex-col space-y-5 card p-10'>
          ...
          <hr />
          <Link to='/Register' className="text-[14px] text-primary font-semibold">NOT YET REGISTRED? CLICK HERE TO REGISTER</Link>
        </div>
      </div>
      {/*bottom corner*/}
      <div className=" flex justify-end">
