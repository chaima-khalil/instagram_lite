...
function Sharepost() {
    ...
    const getData = async () => {
        ...
        const querySnapchot = await getDocs(collection(fireDb, "users"));
        const temp = [];
    ...
    const Sharepost= async ()=>{
        ...
        tempShares.push({...post , sharedBy:JSON.parse(localStorage.getItem("insta-lite")),
    });
       ...
       toast.success('post shared successfuly')
       navigate('/home')
    }
    return (
        <DefaultLayout>
            ...
            {post && data && (<div>
                ...
                <div className='grid grid-cols-5 md:grid-cols-1 gap-5'>
                    {data.map((user) => {
                        ...
                        return <div className={` cursor-pointer card p-3 justify-center items-center flex flex-col  ${alreadySelected && ' border-4 border-primary'}`}
...
<div className='mt-5'>
<button className='bg-primary h-10 rounded-sm text-white px-10  ' onClick={Sharepost}> Sharepost</button>
